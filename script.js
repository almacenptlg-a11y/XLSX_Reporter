class DataViewerApp {
  ...
  initEvents() {
    ...
    document.addEventListener('click', (e) => {
      if (!e.target.closest('#btnColumns') && !e.target.closest('#colMenu')) this.els.colMenu.classList.remove('show');
      if (!e.target.closest('#btnExport') && !e.target.closest('#exportMenu')) this.els.exportMenu.classList.remove('show');
      if (!e.target.closest('#columnContextMenu') && !e.target.closest('.btn-col-menu')) this.els.ctxMenu.classList.remove('show');
    });
  }
  ...
  async handleFile(file) {
    ...
    try {
      ...
      if (workbook.SheetNames.length === 0) throw new Error("Archivo vacío o inválido.");
  ...
  async loadSheetData(sheetName) {
    try {
      ...
      if (rawMatrix.length === 0) throw new Error(`La hoja "${sheetName}" está vacía.`);
  ...
  openStructureSelector() {
    ...
    const likelyHeader = this.tempRawMatrix.findIndex(row => row && row.filter(c => c).length > 1);
  ...
  renderPreviewTableRows() {
    ...
    if(totalRows > limit) {
       ...
       tr.innerHTML = `<td colspan="100" style="text-align:center; padding:4px; font-style:italic; color:var(--text-muted)">... ${totalRows - limit} filas más ...</td>`;
  ...
  applyStructureAndLoad() {
    ...
    setTimeout(() => {
      try {
        ...
        if(!headerRow || headerRow.length === 0) throw new Error("La fila de encabezado seleccionada está vacía.");
        ...
        if(jsonData.length === 0) throw new Error("No se encontraron datos válidos con esta estructura.");
        ...
        this.showToast(`Estructura aplicada. ${jsonData.length} filas cargadas.`, 'success');
    ...
    this.columns.forEach(col => {
      ...
      th.innerHTML = `
        <div class="th-content ${alignClass}">
          <div class="btn-col-menu ${hasFilter ? 'active' : ''}" onclick="app.openColumnMenu(event, '${col}')">
             <i class="ph ${hasFilter ? 'ph-funnel ph-fill' : 'ph-dots-three-vertical'}"></i>
          </div>
          <div class="th-title" onclick="app.sortBy('${col}')">
            <span>${col}</span>
  ...
  renderMenuContent(col, container) {
      ...
      extraControls += `
             <div style="margin-top:8px">
                ...
                <select class="form-select" onchange="app.changeColDateStyle('${col}', this.value)">
                   ...
                   <option value="full" ${settings.dateStyle === 'full' ? 'selected' : ''}>Texto (Lunes...)</option>
                </select>
             </div>
          `;
    }
    ...
    container.innerHTML = `
        <div class="col-menu-section">
          ...
          <select class="form-select" onchange="app.changeColFormat('${col}', this.value)">
            ...
            <option value="link" ${settings.type === 'link' ? 'selected' : ''}>Link</option>
          </select>
          ${extraControls}
        </div>
        <div class="col-menu-section">
    ...
    allDiv.onclick = (ev) => {
      if (ev.target.tagName !== 'INPUT') {
        const chk = allDiv.querySelector('input'); chk.checked = !chk.checked;
      }
    ...
    valsToShow.forEach(val => {
      ...
      div.innerHTML = `<input type="checkbox" class="val-chk" value="${val}" ${isChecked ? 'checked' : ''}> <span>${displayVal}</span>`;
      div.onclick = (ev) => {
        if (ev.target.tagName !== 'INPUT') {
    ...
    if (isLimited) {
      ...
      w.style.fontSize = '10px'; w.style.color = 'var(--text-muted)'; w.innerText = `...y ${uniqueVals.length - maxItems} más.`;
  ...
  filterMenuSearch(term) {
    ...
    items.forEach(el => {
      el.style.display = el.getAttribute('data-val').includes(term) ? 'flex' : 'none';
    });
  }
  ...
  filterColumnList(term) {
    ...
    this.renderColumnList(this.columns.filter(c => c.toLowerCase().includes(lower)));
  }
    ...
    if (format === 'xlsx') {
      ...
      this.visibleData.forEach(row => {
        ...
        this.columns.forEach(col => {
          ...
          } else if (['date', 'datetime'].includes(type) || val instanceof Date) {
            try {
              const d = new Date(val);
              if (!isNaN(d)) {
                const isDateOnly = (type === 'date');
                const iso = d.toISOString();
                const fmtVal = isDateOnly ? iso.split('T')[0] : iso.slice(0, 16);
              ...
              } else { cellHtml = esc(val); }
            } catch (e) { cellHtml = esc(val); }
          } else {
            let txt = String(val);
            if (!isNum && txt.length < 20 && txt.length > 2) {
              const up = txt.toUpperCase();
              let badge = '';
              if (up.match(/APROB|OK|ACTIVO|PAGADO|COMPLET|SI|SÍ/)) badge = 'success';
              else if (up.match(/ERROR|RECHAZ|CANCEL|BAJA|NO|FALLO/)) badge = 'danger';
              else if (up.match(/PEND|REV|PROC|ESPERA/)) badge = 'warning';
              else if (up.match(/NUEVO|INFO/)) badge = 'info';

              if (badge) cellHtml = `<span class="badge ${badge}">${esc(txt)}</span>`;
        ...
        this.columns.forEach((col, idx) => {
          ...
          if (idx === 0) td = '<td>TOTAL</td>';
            ...
            if (totals[col] !== undefined) {
              ...
              td = `<td class="text-right" data-sum="1" data-fmt="${esc(config.currency || '')}">${fmtVal}</td>`;
            } else {
              td = '<td></td>';
            }
          }
      ...
      this.columns.forEach(col => {
        if (this.colSettings[col].hidden) return;
        const isNum = ['number', 'currency', 'integer', 'percent'].includes(this.colSettings[col].type);
        const align = isNum ? 'text-right' : 'text-left';
        headersHtml += `<th class="${align}" onclick="sortGrid(${colIndex})">${esc(col)}</th>`;
...
<body>
  ...
  <div class='page'>
    <div class='header-container'>
      <div class='title-area'>
        <h1>${esc(fname)}</h1>
        <div class='author-pill'><svg width='14' height='14' fill='none' stroke='currentColor' stroke-width='2' viewBox='0 0 24 24'><path d='M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2'></path><circle cx='12' cy='7' r='4'></circle></svg> ${esc(author)} <div class='subtitle-area'>From DataReport By MFPT: ${timestamp}</div></div>
      </div>
      <div class='meta-row'>
    ...
    const a = document.createElement('a'); a.href = url; a.download = `${fname}.html`; a.click();
  } else if (format === 'pdf') {
    ...
    doc.autoTable({
      ...
      startY: 28, theme: 'grid', styles: { fontSize: 8 }, headStyles: { fillColor: [59, 130, 246] }
    });
    doc.save(`${fname}.pdf`);
  }
  ...
  formatValue(val, config) {
    ...
    if (typeof val === 'string') {
      ...
      if (['APROBADO', 'OK', 'SI', 'SÍ', 'ACTIVO', 'PAGADO'].includes(vUpper)) return `<span class="badge badge-success">${val}</span>`;
      if (['ERROR', 'NO', 'BAJA', 'FALLO', 'ANULADO'].includes(vUpper)) return `<span class="badge badge-danger">${val}</span>`;
      if (['PENDIENTE', 'ESPERA', 'PROCESO'].includes(vUpper)) return `<span class="badge badge-warning">${val}</span>`;
      if (val.startsWith('http')) return `<a href="${val}" target="_blank" style="color:var(--accent)">Link</a>`;
      return val;
    }
    ...
    if (typeof val === 'number') {
      if (type === 'currency') return val.toLocaleString('es-PE', { style: 'currency', currency: curr, minimumFractionDigits: decimals, maximumFractionDigits: decimals });
      if (type === 'percent') return val.toLocaleString('es-PE', { style: 'percent', minimumFractionDigits: decimals, maximumFractionDigits: decimals });
...
// --- CUSTOM CSV EXPORT LOGIC --- //
  ...
  openCsvMapper() {
    ...
    const selects = [this.els.mapLocalidad, this.els.mapScanCode, this.els.mapProducto, this.els.mapPedido, this.els.mapOrdenCompra];
    ...
    this.autoSelectOption(this.els.mapLocalidad, ['localidad', 'ciudad', 'sede', 'ubicacion']);
    this.autoSelectOption(this.els.mapScanCode, ['scan', 'code', 'codigo', 'ean', 'sku']);
    this.autoSelectOption(this.els.mapProducto, ['producto', 'descripcion', 'item', 'nombre']);
    this.autoSelectOption(this.els.mapPedido, ['pedido', 'cant', 'solicitud']);
    this.autoSelectOption(this.els.mapOrdenCompra, ['orden', 'compra', 'oc', 'po', 'purchase', 'numero']);
  ...
  toggleAutoOC() {
    ...
    if (isAuto) {
      ...
      const fmt = `${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
  ...
  generateCustomCSV() {
    ...
    csvRows.push("LOCALIDAD,SCAN_CODE,PRODUCTO X,PEDIDO,ORDEN DE COMPRA");
    ...
    this.visibleData.forEach(row => {
      ...
      csvRows.push(`${clean(valLoc)},${clean(valScan)},${clean(valProd)},${clean(valPed)},${clean(valOC)}`);
    });
